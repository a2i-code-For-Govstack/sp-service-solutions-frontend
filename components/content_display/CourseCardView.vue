<template>
    <div class="item_block">
        <a :href="item.view_link_type=='1'?item.content_link:'/show-course/' + item.id" :target="item.view_link_type=='1'?'_blank':'_self'">
            <div class="item">
                <div class="content_img">
                    ...
                    <img :src="$imageUrl('course/partner-'+ item.partner_info.id +'/'+ item.cover_photo_url)" />
                </div>
                ...
                <div class="content_title">
                    ...
                    <p v-else class="content_for"><span v-if="item.education_class_id">{{ item.education_class_id.title_bn }}</span><span v-if="item.education_class_id && item.subject_id"> । </span><span v-if="item.subject_id">{{ item.subject_id.title_bn }}</span></p>
                </div>
            </div>
        </a>
...
<style scoped>
    ...
    .item_block .content_img .icon:before{
        top: -11px;
        left: -11px;
        width: 30px;
        height: 30px;
