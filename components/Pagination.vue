...
export default {
    ...
    methods: {
        total_record_display: function(obj){
            let str = '';
            if(this.sel_lang=='bn') str = 'পাতা ' + this.$enToBnNumber(obj.last_page) + ' এর ' + this.$enToBnNumber(obj.current_page);
            ...
            if(this.data.current_page!==pg){                
                if(this.action!=='') this.action(pg)
                    ...
                    if(pg==1){
                        ...
                        let obj = {};
                        if(this.$route.query) {
                            for(var param in this.$route.query) obj[param] = this.$route.query[param]
                        }
                        obj['page'] = pg;
