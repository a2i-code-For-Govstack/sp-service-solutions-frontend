<template>
    <div class="opinion_poll_block_container">
...
export default {
    ...
    watch: {
        $route(to, from){            
            if(to.query.type) this.sel_polling_type = to.query.type
    ...
    mounted(){
        ...
        if(this.$route.query.type) this.sel_polling_type = this.$route.query.type
        this.load_data();
    },
    beforeDestroy(){
    ...
    methods: {
        ...mapActions({        
            opinionPollSubmit: 'opinion_polls_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/opinion-polls'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.req_from_external_domain && localStorage.getItem('req_from')?'&req_from=' + localStorage.getItem('req_from'):'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'') + (this.sel_polling_type?'&req_status=' + this.sel_polling_type:'');
