...
export default {
    ...
    methods: {
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/categories'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'');
