        ...
        <div class="filter_option_block">
            ...
            <ul>
                <li @click="filter_by_type('')">
                    <i v-if="$parent.sel_survey_type==''" class="far fa-check-circle"></i>
                    <i v-else class="far fa-circle"></i>
                    <span>All</span>
                </li>
                <li @click="filter_by_type(1)">
                    <i v-if="$parent.sel_survey_type==1" class="far fa-check-circle"></i>
                    <i v-else class="far fa-circle"></i>
                    <span>Ongoing</span>
                </li>
                <li @click="filter_by_type(2)">
                    <i v-if="$parent.sel_survey_type==2" class="far fa-check-circle"></i>
                    <i v-else class="far fa-circle"></i>
                    <span>Completed</span>
                </li>
            </ul>
            ...
            <tbody>
                <template v-if="data.length>0">
                    <tr v-for="(item,index) in data" :key="index">
                        ...
                        <td width="20%">{{ item.survey_title }}</td>
                        <td width="25%">
...
export default {
    ...
    methods: {
        filter_by_type: function(val){
            ...
            this.$parent.sel_survey_type = val
        },
        ...
        del_item: function(id){
            if(confirm('Are you sure to delete it?')){
                this.$axios.get('/api/online-surveys/delete/' + id, this.$parent.header_config).then( (response) => {
        ...
        get_explain_list: function(data){
            ...
            this.sel_survey_id = data.survey_id
            this.sel_survey_option_id = data.id
        },
        close_popup(){
