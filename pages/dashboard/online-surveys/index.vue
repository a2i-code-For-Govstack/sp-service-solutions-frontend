<template>
    <div class="online_survey_block_container">
...
export default {
    name: 'OpinionPollBlockContainer',    
    components: {
        ContentEntryForm,
        ContentList        
    },
    ...
    watch: {
        $route(to, from){            
            if(to.query.type) this.sel_survey_type = to.query.type
            this.load_data(to.query.page?to.query.page:1);
        },
        get_search_info: function(){
            this.load_data();
        },
        sel_survey_type: function(){
            this.load_data();
        }
    },
    computed: {
    ...
    mounted(){
        ...
        if(this.$route.query.type) this.sel_survey_type = this.$route.query.type
        this.load_data();
    },
    beforeDestroy(){
    ...
    methods: {
        ...mapActions({        
            onlineSurveySubmit: 'online_survey_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/online-surveys'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.req_from_external_domain && localStorage.getItem('req_from')?'&req_from=' + localStorage.getItem('req_from'):'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'') + (this.sel_survey_type?'&req_status=' + this.sel_survey_type:'');
