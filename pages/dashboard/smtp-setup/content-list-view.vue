            ...
            <tbody>
                <template v-if="data.length>0">
                    <tr v-for="(item,index) in data" :key="index">
                        ...
                        <td>
                            <div v-if="item.domain_group_info">Group: {{ item.domain_group_info.group_title }}</div>
                            <div v-if="item.mail_host">Host: {{ item.mail_host }}</div>
                            <div v-if="item.mail_port">Port: {{ item.mail_port }}</div>
                            <div v-if="item.mail_username">Username: {{ item.mail_username }}</div>
                            <div v-if="item.mail_password">Password: {{ item.mail_password }}</div>
                            <div v-if="item.mail_encryption">Encryption: {{ item.mail_encryption }}</div>
                        </td>
                        <td align="center">
                            <span class="status">
    ...
    methods: {
        ...
        del_item: function(id){
            if(confirm('Are you sure to delete it?')){
                this.$axios.get('/api/smtp-setup/delete/' + id, this.$parent.header_config).then( (response) => {
