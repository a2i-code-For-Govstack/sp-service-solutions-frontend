<template>
    <div class="smtp_setup_block_container">
...
export default {
    ...
    methods: {
        ...mapActions({        
            smtpSetupSubmit: 'smtp_setup_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/smtp-setup'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'');
