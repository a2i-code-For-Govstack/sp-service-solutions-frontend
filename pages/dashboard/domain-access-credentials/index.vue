<template>
    <div class="domain_access_credentials_list_block_container">
...
export default {
    name: 'DomainAccessCredentialListBlockContainer',    
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({        
            domainAccessCredentialSubmit: 'domain_access_credential_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/domain-access-credentials'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'');
