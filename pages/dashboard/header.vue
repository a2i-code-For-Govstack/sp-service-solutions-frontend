<template>
    <div class="header_container">
            ...
            <div class="srch_block">
                <template v-if="$parent.title!==$store.state.dashboard.prefix">
                    ...
                    <div class="srch_input_block">
                        <input type="text" placeholder="search for an entry..." v-model="$parent.keyword" @keyup.enter="req_search()" />
                    </div>
                </template>
                <template v-else>
        ...
        <div class="user_login_info">
            <template v-if="!$store.state.auth_req_from_others">
                <div class="photo">
...
export default {
    ...
    watch: {
        $route(to, from ){
            if(to.query.keyword) this.$parent.keyword = to.query.keyword
            this.req_search()
        }
    },
    methods: {        
        ...
        req_search: function(){
            this.search_status(this.$parent.keyword)
            if(this.$parent.keyword){
                let obj = {}
                obj['keyword'] = this.$parent.keyword            
