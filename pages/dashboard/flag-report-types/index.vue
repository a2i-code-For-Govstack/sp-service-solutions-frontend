<template>
    <div class="flag_rpt_type_block_container">
...
export default {
    name: 'FlagRptTypesBlockContainer',    
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({        
            flagRptTypeSubmit: 'flag_rpt_type_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/flag-rpt-types'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'');
