<template>
    <div class="domain_groups_block_container">
...
export default {
    name: 'DomainGroupsBlockContainer',    
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({        
            domainGroupSubmit: 'domain_group_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/domain-groups'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$route.query.keyword?'&keyword=' + this.$route.query.keyword:'');
