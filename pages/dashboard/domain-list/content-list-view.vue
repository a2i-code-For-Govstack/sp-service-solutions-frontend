            ...
            <tbody>
                <template v-if="data.length>0">
                    <tr v-for="(item,index) in data" :key="index">
                        ...
                        <td>{{ item.sitename_bn }} ({{ item.sitename_en }})</td>
                        <td>
                            <!-- <div v-if="item.domain_id">ID: {{ item.domain_id }}</div> -->
                            <div v-if="item.domain_group_info">Group: {{ item.domain_group_info.group_title }}</div>
                            <div>Subdomain: {{ item.sub_domain }}</div>
                            <div v-if="item.alias">Alias: {{ item.alias }}</div>
                        </td>
                        <!-- <td>{{ item.cluster }}</td> -->
                        <td align="center">
                            <span class="status">
    ...
    methods: {
        ...
        del_item: function(id){
            if(confirm('Are you sure to delete it?')){
                this.$axios.get('/api/domain-list/delete/' + id, this.$parent.header_config).then( (response) => {
