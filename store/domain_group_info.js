...
export const actions = {
    async GET_DATA ({ commit }, request) {
        ...
        console.log('Domain group info data request')      
        try {
            const { data } = await this.$axios.get('/api/domain-groups' + (request['limit']>0?'/' + request['limit']:'') + (request['page']?'?page=' + request['page']:''), {
                headers: {
    ...
    async DATA_SUBMIT ({ commit }, request) {
        ...
        try {
            const { data } = await this.$axios.post('/api/domain-groups' + (request['action']=='update'?'/update/' + request['edit_id']:'/store'), request.data, {
                headers: {
                    'Authorization': 'Bearer ' + request.access_token,
                    'Content-Type': 'application/json',
                    'X-XSRF-TOKEN': request.access_token
